...
class PdfToText extends PdfObjectBase
{
    ...
    public function __next_instruction($page_number, $data, $data_length, $index, $current_template)
    {
    ...
    public function __next_token($page_number, $data, $data_length, $index)
    {
        // Skip spaces
        ...
        switch ($ch) {
            ...
            default:
                ...
                if (isset($data[$index])) {
                    ...
                    } elseif ((isset(self::$CharacterClass[$ch]) && (self::$CharacterClass[$ch] & self::CTYPE_ALPHA)) ||
                            $ch == '/') {
                        $ch = $data[$index];
    ...
    protected function GetTrailerInformation($contents)
    {
        ...
        $this->EncryptionAlgorithm = (int) $algorithm_match['value'];

        preg_match('#/R \s+ (?P<value> \d+)#ix', $encryption_data, $algorithm_revision_match);
        $this->EncryptionAlgorithmRevision = (int) $algorithm_revision_match['value'];

        preg_match('#/P \s+ (?P<value> \-? \d+)#ix', $encryption_data, $flags_match);
        $this->EncryptionFlags = (int) $flags_match['value'];
...
class PdfTexterFontTable extends PdfObjectBase
{
    ...
    public function Add($object_id, $font_definition)
    {
        ...
        elseif (preg_match('#/Encoding \s* /WinAnsiEncoding#ix', $font_definition)) {
        ...
        elseif (preg_match('#/Encoding \s* /MacRomanEncoding#ix', $font_definition)) {
            $font_type = PdfTexterFont::FONT_ENCODING_MAC_ROMAN;
        }
        // Experimental implementation of CID fonts
        elseif (preg_match('#/Encoding \s* /Identity-H#ix', $font_definition)) {
...
class PdfTexterEncodingMap extends PdfTexterCharacterMap
{
    ...
    private static $Encodings = [
        ...
        'odblacute' => [0x151, 0x151, 0x151, 0x151],
        'udblacute' => [0x171, 0x171, 0x171, 0x171],
        'Odblacute' => [0x150, 0x150, 0x150, 0x150],
        'Udblacute' => [0x170, 0x170, 0x170, 0x170],
       ];
    ...
    public function offsetGet($offset)
    {
        if (!$this->Secondary) {
            ...
            elseif (isset(self::$Encodings[$ord][$this->Encoding])) {
    ...
    public function __construct($object_id, $map_name)
    {
        ...
        if (isset(self::$CachedMaps[$map_name])) {
            ...
            if (!file_exists($file)) {
                ...
                self::$CachedMaps[$map_name] = ['file' => $file, 'map' => $map];
            }
        }
...
class PdfInlinedImage extends PdfImage
{
    ...
    public static function CreateInstance($stream_data, $object_data, $autosave)
    {
        ...
        return  new self($image_data, $width, $height, $bits_per_component, $color_space);
    }
